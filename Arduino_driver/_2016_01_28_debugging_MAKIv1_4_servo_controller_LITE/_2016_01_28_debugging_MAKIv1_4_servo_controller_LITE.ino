...
#define SC_SET_TM  "TM"  // servo command syntax for setting a specified servo's MAX TORQUE 
#define SC_SET_TL  "TL"  // servo command syntax for setting a specified servo's TORQUE LIMIT
#define SC_SET_TS  "TS"  // servo command syntax for setting a specified servo's TORQUE ENABLE
...
unsigned int EC_timer_duration = 2000;  //30000;  // ms, or 30s
...
boolean print_debugging_flag = true;
...
void setup()
{
  ...
  for (nServo=1; nServo <= SERVOCOUNT; nServo++)  { 
     ...
     // reset EEPROM values; factory default is 36
     // see http://support.robotis.com/en/techsupport_eng.htm#product/dynamixel/ax_series/dxl_ax_actuator.htm
     ax12SetRegister(nServo, AX_ALARM_LED, ERR_NONE);
     ax12SetRegister(nServo, AX_ALARM_SHUTDOWN, ERR_NONE); 
   }
   Serial.flush();
  ...
  //KATE debugging
  Serial.print("\nBEFORE HP AX_CW_ANGLE_LIMIT_L: ");
  ax12SetRegister2(HP, AX_CW_ANGLE_LIMIT_L, 400);
  Serial.print("; AFTER HP AX_CW_ANGLE_LIMIT_L: ");
  Serial.print( ax12GetRegister(HP, AX_CW_ANGLE_LIMIT_L, 2) );
  Serial.print("\nBEFORE HP AX_CCW_ANGLE_LIMIT_L: ");
  ax12SetRegister2(HP, AX_CCW_ANGLE_LIMIT_L, 600);
  Serial.print("; AFTERHP AX_CCW_ANGLE_LIMIT_L: ");
  Serial.print( ax12GetRegister(HP, AX_CCW_ANGLE_LIMIT_L, 2) );
  Serial.print("\n");
  Serial.flush();
  ...
  // KATE debugging
  Serial.print("\nSet HP AX_GOAL_POSITION_L out of range; expect Angle Limit Error");
  ax12SetRegister2(HP, AX_GOAL_POSITION_L, 300);
  int my_error = ax12GetLastError();  // should be from the most recent ax12GetRegister call
  Serial.print("\nStatus: ");
  Serial.print(my_error);
  Serial.print(", ");
  Serial.print(my_error, BIN);
  Serial.print("\n");
  Serial.flush();
  my_error = ax12GetRegister(HP, AX_ALARM_LED, 1);
  Serial.print("\nHP AX_ALARM_LED: ");
  Serial.print(my_error);
  Serial.print(", ");
  Serial.print(my_error, BIN);
  Serial.print("\n");
  Serial.flush();
  my_error = ax12GetLastError();  // should be from the previous ax12GetRegister call
  Serial.print("\nStatus: ");
  Serial.print(my_error);
  Serial.print(", ");
  Serial.print(my_error, BIN);
  Serial.print("\n");
  Serial.flush();
  my_error = ax12GetRegister(HP, AX_ALARM_SHUTDOWN, 1);
  Serial.print("\nHP AX_ALARM_SHUTDOWN: ");
  Serial.print(my_error);
  Serial.print(", ");
  Serial.print(my_error, BIN);
  Serial.print("\n");
  ...
  Serial.print("\nSet HP AX_TORQUE out of range; expect Range Error");
  ax12SetRegister(HP, AX_TORQUE_ENABLE, 5);
  my_error = ax12GetRegister(HP, AX_ALARM_LED, 1);
  if (my_error == -1)  {
    my_error = ax12GetRegister(HP, AX_ALARM_LED, 1);
  }
...
void loop() {
  ...
  if (print_debugging_flag)  {  // KATE debugging
  ...
  while (tmp_string.length() > 0)  {
    ...
    { 
     ...
     if (tmp_string.startsWith(SC_GET_PP)  ||
      ...
      //Serial.println(parsed_int);    //debugging
      index += tmp_i;              
      setServoMaxTorque(target_motor, parsed_int);  //KATE
      ...
      //Serial.println(parsed_int);    //debugging
      index += tmp_i;              
      setServoTorqueLimit(target_motor, parsed_int);  //KATE

    } else if (tmp_string.startsWith(SC_SET_TS))  {
      unsigned int tmp_i = parseSCDIntIndex(tmp_string.substring(2));
      //unsigned int tmp_i = 1;  // state is boolean; 0=disabled, 1=enabled
      parsed_int = (tmp_string.substring(2,2+tmp_i+1)).toInt();
      Serial.println(parsed_int);    //debugging
      index += tmp_i;              
      setServoTorqueState(target_motor, parsed_int);  //KATE
...
void pingServos()  {
  // see http://support.robotis.com/en/techsupport_eng.htm#product/dynamixel/ax_series/dxl_ax_actuator.htm
  int temp;
  int length = 0 + 2;  // number of parameters(N) + 2
  int checksum;  // checksum = ~(id + length + instruction + param1 +...+paramN)
  for (int nServo=1; nServo<=SERVOCOUNT; nServo++)
  {  
    checksum =~ (nServo + length + AX_PING + 0);
    Serial.print("checksum=");
    Serial.print(checksum);
    Serial.print("\tchecksum=");
    Serial.print(checksum, HEX);
    Serial.print("\tmanual=");
    Serial.print(nServo + length + AX_PING + 0, HEX);
    Serial.print("\n");
    Serial.flush();
    
    setTX(nServo);    // declare transmission to specific servo
    ax12write(0xFF);  // header
    ax12write(0xFF);
    ax12write(nServo);  // id
    ax12write(length);  // length
    ax12write(AX_PING);  // instruction
    //ax12write(0xff - (checksum % 256));  // checksum
    ax12write(checksum);
    setRX(nServo);    // declare transmission to specific servo complete
  }
}  // end  pingServos
...
void readMotorValues(String feedbackType)  {
  ...
  if (my_verbose_debug)  Serial.print("\n");
...
int errorCheckMotor(int nServo, boolean PE)  {
  ...
  /*
  int my_error = 36;  // default value, 00100100
  ...
  */
  ...
  int my_error = ax12GetRegister(nServo, AX_ALARM_SHUTDOWN, 1);
  Serial.print("BEFORE: my_error = ");
  Serial.print(my_error);
  Serial.flush();

  if (my_error == -1)  {
    //didn't readPacket successfully
    //try again
    my_error = ax12GetRegister(nServo, AX_ALARM_SHUTDOWN, 1);
  }
  ...
  if ( (ax12GetRegister(nServo, AX_PRESENT_LOAD_L, 2) == 0) &&
    ...
    Serial.print("AFTER: my_error = ");
    Serial.print(my_error);
    Serial.flush();
    
    //KATE debugging
    print_debugging_flag = false;
  }
  ...
  if (my_error == ERR_NONE) {
    ...
    if ((my_error & ERR_OVERLOAD) == ERR_OVERLOAD)  {
      ...
      if (ax12GetRegister(nServo, AX_TORQUE_LIMIT_L, 2) == 0)  {  // superfluous since we did this check above
    ...
    if (nServo >= HT)  {    // KATE debugging
    if (print_errorString && PE)  {
