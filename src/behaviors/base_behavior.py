...
## 2016-06-16, KATE
	...
	def requestFeedback( self, feedback_type, cmd_prop=True, time_ms=100, time_inc=0.01 ):
		...
		## request a feedback message
		baseBehavior.pubTo_maki_command( self, feedbackCommand(feedback_type), cmd_prop=cmd_prop, time_ms=time_ms, time_inc=time_inc )
		return
	...
	def initSubMAKIFeedback( self ):
		rospy.Subscriber('maki_feedback', MakiFeedback, handleMakiFeedback)

	def handleMakiFeedback ( self, feedback ):
        self.makiFeedbackValues = feedback
...
class headPanBaseBehavior(baseBehavior):
	...
	def amend_maki_command( self, commandOut, fixed_gaze=True ):
        headPanGoalPos = getSetCommandSetting(commandOut, HEAD_PAN, GOAL_POS)
        eyePanGoalPos = getSetCommandSetting(commandOut, EYE_PAN, GOAL_POS)
        if fixed_gave and headPanGoalPos and not eyePanGoalPos:
            eyePan = self.autoCalculateFixedGazeFromHPGP(headPanGoalPos)
            commandOut = updateSetCommand(commandOut, EYE_PAN, GOAL_POS, eyePan)
			rospy.logdebug("headPanBaseBehavior.pubTo_maki_command(): Updated commandOut to: " + str(commandOut))
		return commandOut
	...
	def autoCalculateFixedGazeFromHPGP( self, hp_gp ):
		...
		_hp_gp_degrees = DX_Conv.convertToDegrees_ticks( abs(hp_gp - HP_FRONT) ) 
		_ep_gp_degrees = _hp_gp_degrees * self.aCFG_coeff
		ret = DX_Conv.convertToTicks_degrees( _ep_gp_degrees )
		if (hp_gp < HP_FRONT):
			ret = EP_FRONT - ret
		else:
			ret = EP_FRONT + ret
...
class headTiltBaseBehavior(baseBehavior):
	...
	def __init__(self, verbose_debug, ros_pub):
		...
		_maki_feedback_sub[ str(PRESENT_TEMP) ] = ("maki_feedback_pres_temp", String, self.parseMAKIFeedbackMsg)
		...
		if headTiltBaseBehavior.__ht_enable_cmd == None:
			headTiltBaseBehavior.__ht_enable_cmd = setCommand(HEAD_TILT, TORQUE_LIM, ht_tl_enable)
		if headTiltBaseBehavior.__ht_disable_cmd == None:
			headTiltBaseBehavior.__ht_disable_cmd = setCommand(HEAD_TILT, TORQUE_LIM, ht_tl_disable)
		try:
	...
	def monitorMotorTemperature( self ):
		...
		headTiltBaseBehavior.requestFeedback( self, PRESENT_TEMP )

		## save our initial state
		_start_pt.update( self.makiFeedbackValues[ PRESENT_TEMP ] )
		_current_pt.update( self.makiFeedbackValues[ PRESENT_TEMP ] )
		_previous_pt.update( self.makiFeedbackValues[ PRESENT_TEMP ] )
		_running_sum_pt.update( self.makiFeedbackValues[ PRESENT_TEMP ] )
		_running_average_pt = dict( zip(F_VAL_SEQ, [ 0 ] * len(F_VAL_SEQ) ) )
		_delta_pt.update( self.computeTemperature( self, _current_pt, _previous_pt, "delta" ) )
		_average_pt.update( self.computeTemperature( self, _current_pt, _previous_pt, "average" ) )
		## TODO
...
## 2016-06-16, KATE
		while (not rospy.is_shutdown()) and self.ALIVE:
			...
			_delta_pt = self.computeTemperature( self, _current_pt, _previous_pt, "delta" )
			_average_pt = self.computeTemperature( self, _current_pt, _previous_pt, "average" )
			_running_sum_pt = self.computeTemperature( self, _running_sum_pt, _current_pt, "add" )
			_running_average_pt = self.computeTemperature( self, _running_sum_pt, _loop_count, "running_average" )
...
## 2016-06-16, KATE
	...
	def enableHT( self ):
		if headTiltBaseBehavior.__ht_enabled == True:
			...
			headTiltBaseBehavior.pubTo_maki_command( self, headTiltBaseBehavior.__ht_enable_cmd, cmd_prop=False )
			return
		...
		if (self.makiPP["HT"] <= HT_MAX and self.makiPP["HT"] >= HT_MIN):
			_pub_cmd_GP = setCommand(HEAD_TILT, GOAL_POS, self.makiPP["HT"])
			...
			baseBehavior.pubTo_maki_command( self, _pub_cmd_GP )
## 100ms
		...
		while (not rospy.is_shutdown()) and (not headTiltBaseBehavior.__ht_enabled):	## value set in parseMAKIFeedbackMsg
			if (_loop_count == 0):
				headTiltBaseBehavior.pubTo_maki_command( self, headTiltBaseBehavior.__ht_enable_cmd )
## 100ms
...
class eyelidBaseBehavior( baseBehavior ):
	...
	def setEyelidNeutralPose( self, ll, monitor=False, cmd_prop=False):
		...
		self.origin_ll = ll
		_pub_cmd = setCommand(EYELID_LEFT, GOAL_POS, self.origin_ll) 
...
## 2016-06-16, KATE
	...
	def eyelidClose( self, ll_close=None, monitor=False, cmd_prop=False ):
        _pub_cmd = setCommand(EYELID_LEFT, GOAL_POS, ll_close or self.ll_close)
...
## 2016-06-16, KATE
	...
	def eyelidOpen( self, ll_open=None, monitor=False, cmd_prop=False ):
		_start_time = rospy.get_time()
        _pub_cmd = setCommand(EYELID_LEFT, GOAL_POS, ll_open or self.ll_open)
